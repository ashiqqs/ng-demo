...
export class SessionDbService<TEntity> implements IRepository<TEntity> {
  ...
  private async findAsObj(): Promise<TEntity[]> {
    return new Promise((resolve, reject) => {
      const data = sessionStorage.getItem(this.table);
      if (data) {
  ...
  async create(item: TEntity): Promise<TEntity> {
    return new Promise((resolve, reject) => {
      ...
      if (data) {
        ...
        sessionStorage.setItem(this.table, JSON.stringify(dataAsObj));
        resolve({
          id,
          ...item,
        });
        ...
        sessionStorage.setItem(this.table, JSON.stringify(firstItem));
        resolve({ ...item, id: 1 });
      }
    });
  }
